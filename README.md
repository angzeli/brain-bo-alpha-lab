...
Current template families include price momentum/reversion, low volatility, volume ratio/surprise, inverse volume ratio, range position, time-series rank, short/long trend, price-volume momentum/reversal, smoothed price and price-volume variants, high/low momentum spread, close-to-VWAP momentum, and intraday position. Future suggestions avoid unsupported `ts_min` and `ts_max`; `ts_scale` and `ts_rank` are used because they are confirmed available in our current BRAIN setup.
